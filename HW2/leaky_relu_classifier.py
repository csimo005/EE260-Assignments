...
class Model(coatl.module):
    def __init__(self, k):
        ...
        self._activation = layers.relu(alpha=0.01)
...
def zNormalize(dset):
    mean = np.mean(dset._images, axis=0)
    std = np.std(dset._images, axis=0)
    std[std==0] = 1 #if std is 0, then x-m=0, so divide by 1 avoids errors but changes no value
    dset._images = (dset._images-mean)/std

def main(batchsize=10, lr=0.01, epochs=10, k=5, frac_dset=1., fname=''):
    tform = transforms.transformation([transforms.ToTensor()]) #Convert image to tensor
...
if __name__ == '__main__':
    ...
    for i in range(len(k)):
        main(batchsize=1, lr=0.01, epochs=5, k=k[i], frac_dset=5./6, fname='leaky_relu_k%d.txt'%k[i])
